...
function getColorNameFromRoute(routeColor: string): string {
  ...
  const byName = ROUTE_COLOR_OPTIONS.find((c) => c.name.toLowerCase() === routeColor.toLowerCase());
  if (byName) return byName.name;
  const byHex = ROUTE_COLOR_OPTIONS.find((c) => c.hex.toLowerCase() === routeColor.toLowerCase());
  return byHex?.name ?? 'black';
}

function parseRopeGrade(grade: string): { base: string; modifier: '' | '+' | '-' } {

  if (grade === 'competition' || grade === 'COMPETITION') return { base: grade, modifier: '' };
...
export default function RouteEditPopup({ route, onCancel, onSave }: RouteEditPopupProps) {
  ...
  const [editRopeBase, setEditRopeBase] = useState(
    route.grade?.startsWith('5') || route.grade === 'competition' ? parseRopeGrade(route.grade).base : ''
  );
